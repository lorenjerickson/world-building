...
import { CampaignArtifact, deleteCampaignArtifact, loadCampaignArtifacts } from "@/lib/artifact-deletion";
import { AppBreadcrumbs } from "@/components/app-breadcrumbs";
import { WorldSummary, worldApi } from '@/lib/world-entities';
...
export function ActivityIndex({ type }: { type: "worlds" | "campaigns" | "sessions" }) {
  ...
  useEffect(() => {
    ...
    if (type === 'worlds') void worldApi<WorldSummary[]>('').then(setWorlds).catch(() => setWorlds([]));
  }, [type]);
