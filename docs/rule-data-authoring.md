...
Implementation status (July 23, 2026): the bounded resolution slice now publishes immutable artifacts and supports provenance-preserving die-result effects. Checks select reusable Die-derived traits, counted mixed pools, or a complete Dice Roll trait whose compiled collection supplies the whole pool. Guided authoring derives sides and complete pool notation from actual trait contracts; publication cross-validates normalized sides, dice identities, and counts against the compiled traits. Runtime modifiers can add dice, replace matching original results, or increase matching die results. Traits and active effects activate their associated modifiers automatically; explicit modifier IDs remain a compatibility override. Runtime callers may supply high-level trait IDs as shorthand or stable named trait instances. The compiled activation graph expands unconditional requirements, counted additions, and instance-selected `any` prerequisite branches before modifier activation. Repeated copies keep independent, contract-validated value bags; mechanics can read a deliberately named instance with `trait-instance-field` or address its role in the composed contract with `trait-path-field`. Structural reads such as `self.speed.walk.rate` resolve effective values without exposing generated instance IDs; repeated reads such as `self.dice[].sides` require a one-based ordinal. A shared guided scalar-expression control applies these choices consistently to check bonuses and targets, modifier values, validation operands, resource costs, event payload values, and return data while preserving older simple drafts. Checks, modifiers, and operations can declare the traits guaranteed on `self`; completion is then restricted to that recursive contract, publication validates the references, and runtime enforces the requirement without granting it. Operations automatically inherit subject requirements from every check they perform, record direct and inherited provenance in the compiled artifact, and use the effective union for completion, previews, publication, and runtime entry checks. Each completion explains its guaranteeing root, trait chain, contract origin, and inherited check source without persisting fragile presentation metadata. Unavailable saved paths distinguish missing catalog support from contract conflicts and offer only candidate roots proven to compose cleanly and restore the exact path. Conditional modifiers retain local contracts. Trait value modifiers preserve `self`/`this` anchoring, resolve exact mounted paths, stack deterministically, and report source plus before/after provenance. Thus D10 supplies fixed sides locally while Boots of Striding can modify the mounted `speed.walk.rate`. Repeated modifier paths require an explicit all-entries or ordinal selector, validated through the collection's accepted base trait and reported on every affected instance. Resolution modifiers target one check, a roll purpose, a complete Dice Roll trait, or all rolls, after which result selectors narrow individual dice. Every result retains trait, roll-trait source, origin, raw/effective value, replacement links, applied modifiers, and activation provenance through the root trait and instance chains. Hit, damage, saving, and other purposes remain separate subtotals, so bonus damage attached to an attack does not alter the hit target-number total. Transactional execution, deeper repeated-element paths, and outbox integration remain future work.
